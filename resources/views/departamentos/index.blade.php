...
@section('titulo') Listado de departamentos @stop
...
@section('contenido')
	...
	<div class="panel panel-flat">
		...
		<table class="table datatable-basic" id="tabla">
			...
			<tbody>
				...
				<tr>
					...
					<td>{{ $departamento->notas }}</td>
					<td class="text-center">
                        <a href="{{ URL::route('departamentos.edit', $departamento->id) }}" class="tooltip-success editar" data-rel="tooltip" title="Editar {{ $departamento->nombre }}" objeto="{{ $departamento->nombre }}" style="text-decoration:none;"> 
                            <span class="btn btn-sm btn-warning"> <i class="icon-pencil7"></i> </span> 
                        </a>
                        &nbsp;
                        <a href="#" data-id="{{ $departamento->id }}" class="tooltip-error borrar" data-rel="tooltip" title="Eliminar {{ $departamento->nombre }}" objeto="{{ $departamento->id }}"> 
		...
		{!! Form::open(array('route' => array('departamentos.destroy', 'USER_ID'), 'method' => 'DELETE', 'role' => 'form', 'id' => 'form-delete')) !!}
        {!! Form::close() !!}
	</div>
	<!-- /basic datatable -->
@stop
